...
# Página del algoritmo Apriori
    ...
    if st.button("Generar reglas de asociación"):
        ...
        try:
            ...
            # Graficar la red de reglas de asociación
            fig2, ax2 = plt.subplots(figsize=(10, 6))
            ax2 = nx.DiGraph()

            for rule in rules:
                ax2.add_edge(str(list(rule.items)), str(list(rule.ordered_statistics[0].items_add)))

            nx.draw(ax2, with_labels=True)
            st.pyplot(fig2)
            ...
            ax1.bar(items, counts)
            ax1.set_xlabel('Elemento')
            ax1.set_ylabel('Cantidad')
            ax1.set_title('Cantidad de cada elemento en el archivo')
...
# Página de Métricas de Distancia
    ...
    clave_api = 'AIzaSyCRoRdEBeMZUfx_kSjmB-Dgezk2jYWh7bQ'  # Reemplaza con tu clave API de Google Maps
    ...
    @st.cache  # Almacena en caché los resultados de la función
